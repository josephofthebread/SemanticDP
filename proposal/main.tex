...
\end{equation}
where $a$ collects the intercept, mechanism fixed effects, and a random
intercept per base model, and $e_i$ is the error term. $Y$, $\Sdist$, and
$f(\eps)$ are z-scored before fitting, so $b$ and $c$ are
standardized effects on a common scale and can be compared directly.
...
predicts degradation more strongly than the nominal privacy dose does.
(Because $\eps$ is ordinal and mechanism-specific, $b$ is estimated per
mechanism; the comparison is a Wald test per mechanism, so this hypothesis may
be underpowered and is therefore secondary.)
\end{hypothesis}
...
mechanism alone cannot separate $b$ from $c$. We need variation in
...
model~\eqref{eq:model}, $\eps$ is only an ordinal, mechanism-specific dose
variable (interacted with mechanism fixed effects). \emph{$\Sdist$ is the
common axis across mechanisms.} If alignment degradation is predicted better
...
$3$ models $\times$ $4$ mechanisms $\times$ $4$ dose levels $\times$ $3$ seeds
$= 144$ runs ($108$ excluding M0, which has no dose axis; M0 runs
...
\begin{enumerate}[left=0pt,itemsep=1pt,topsep=1pt]
 ...
 \item The mediation decomposition: proportion of the total $\eps$-effect
 (within each mechanism) carried by the $\Sdist$ path, with
 bootstrap CIs over seeds.
 \item Privacy--distortion--alignment curves: $Y$ vs.\ dose and $Y$ vs.\
...
M2 & Mechanisms: M1--M3 implemented; \textbf{DP-SGD/LoRA per-example-gradient
 ...
 at matched dose across mechanisms; pre-registration commit. \\
